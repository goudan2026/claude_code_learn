...
class MessageBus:
    def send(self, frome_agent: str, to_agent: str, content: str, msg_type: str = "message"):
        msg = {
            "from": from_agent,
            "to": to_agent,
            "content": content,
    ...
    def read_inbox(self, agent: str) -> list[dict]:
        ...
        if not inbox.exists():
            return []
        msgs = [json.loadsp(line) for line in inbox.read_text().splitlines() if lines.strip()]
        inbox.unlink()
...
def spawn_teammate_thread(name: str, role: str, prompt: str) -> str:
    if name in active_teammates:
        return f"Teammate {name} is already active"

    system = (
        f"You are "{name}", a {role}."
        f"Use tools to complete tasks."
        f"Send results via send_messages to 'lead'."
    )
    ...
    def run():
        ...
        sub_tools = [
            ...
            {
                "name": "send_messages",
                "description": "Send messages to another agent.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "to": {
        ...
        sub_handlers = {
            ...
            "send_messages": lambda to, content: (BUS.send(name, to, content), "Sent")[1],
        }
